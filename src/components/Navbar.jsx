...
const Navbar = () => {
  ...
  const linkClass = scrolled
    ? "text-sm font-medium text-slate-700 hover:text-slate-950 transition"
    : "text-sm font-medium text-white/90 hover:text-white transition";

  const brandClass = scrolled ? "text-slate-900" : "text-white";
    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${headerClass}`}>
      <nav className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <button onClick={() => scrollToSection("home")} className={`font-semibold tracking-wide ${brandClass}`}>
          SunTouch<span className="text-sky-600">IT</span>
        </button>
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${open ? "max-h-80" : "max-h-0"}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-4">
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-3 flex flex-col gap-2">
            <button onClick={() => scrollToSection("about")} className="text-left px-3 py-2 rounded-xl hover:bg-slate-100 text-slate-800">
              About
            </button>
            <button onClick={() => scrollToSection("services")} className="text-left px-3 py-2 rounded-xl hover:bg-slate-100 text-slate-800">
              Services
            </button>
            <button onClick={() => scrollToSection("gallery")} className="text-left px-3 py-2 rounded-xl hover:bg-slate-100 text-slate-800">
              Gallery
            </button>
            <button onClick={() => scrollToSection("contact")} className="text-left px-3 py-2 rounded-xl hover:bg-slate-100 text-slate-800">
              Contact
            </button>
